...
class MAIA2Transformer(torch.nn.Module):
    def __init__(self, output_dim, elo_dict, cfg):
        ...
        self.token_projection = nn.Linear(cfg.token_dim, cfg.dim_vit)  # cfg.token_dim = token size (e.g., 19)

        # Transformer: processes square tokens and player Elo embeddings
        heads = 16
        dim_head = 64
...
def train_chunks(cfg, data, model, optimizer, all_moves_dict, criterion_maia, criterion_side_info, criterion_value):
    ...
    return round(avg_loss / step, 3), round(avg_loss_maia / step, 3), round(avg_loss_side_info / step, 3), round(
        avg_loss_value / step, 3)
